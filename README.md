...
- 🎵 Многоисточниковый музыкальный плеер: поддержка YouTube, Spotify, Soundcloud, Yandex.Music и прямых HTTP потоков.
...
- Многоисточниковый музыкальный плеер с поддержкой Spotify, Soundcloud, Yandex.Music и HTTP потоков;
- Система модерации с многоязычным фильтром мата, прогрессивной системой наказаний и командой `/infopol`;
- Небольшой dashboard с кнопками/вкладками (личный кабинет, государственная дума / голосования, магазин (в разработке)).

Дополнительно см. [MUSIC_PLAYER.md](./MUSIC_PLAYER.md) для документации по музыкальному плееру.
...
Запуск локально
----------------
1. Создайте локально (на сервере) приватный файл `.env` и поместите в него необходимые секреты и идентификаторы. Никогда не публикуйте реальные токены или `.env` в репозитории или в общедоступных каналах.
