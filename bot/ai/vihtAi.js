...
function cannedResponse(prompt) {
  ...
  if (/\b(скачать|download|install|установить|ссылка|где|как|загрузить|получить)\b.*\b(скачать|download|приложение|app|android|ios|windows|виндовс|платформа)\b/i.test(p) || /\b(скачать|download|install|установить|ссылка)\b/i.test(p)) {
    ...
    if (/android|плей\s*маркет|play\s*store/i.test(p)) {
      return `📲 **Для Android:**\nhttps://play.google.com/store/apps/details?id=com.v2raytun.android&hl=ru\n\nПосле установки: откройте приложение V2RayTUN, перейдите на https://vihtai.pro, авторизуйтесь через Telegram и создайте ключ для вашего устройства.`;
    }
    // iOS
    ...
    if (/windows|win|виндовс/i.test(p)) {
      return `💻 **Для Windows:**\nhttps://v2raytunvpn.cc/files/xraysurf.zip\n\nПосле скачивания: распакуйте архив, запустите приложение XRaySurf, перейдите на https://vihtai.pro, авторизуйтесь через Telegram и создайте ключ для вашего устройства.`;
    }
    // Generic downloads page
    return `🔗 **Все приложения и клиенты:** https://vihtai.pro/downloads\n\nВыбери свою платформу (Android, iOS или Windows), скачай нужное приложение (V2RayTUN для Android/iOS или XRaySurf для Windows), затем авторизуйся на https://vihtai.pro через Telegram и создай ключ для вашего устройства.`;
  }
