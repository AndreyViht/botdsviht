  ...
  try {
    ...
    if (customId === 'music_register') {
      try {
        ...
        // Отправляем уведомление о занятости плеера в ветку
        try {
          const threadCh = await client.channels.fetch('1446846080573771877').catch(() => null);
          if (threadCh) await threadCh.send(`🔒 Плеер занят пользователем <@${user.id}> на сервере **${guild.name}**`);
        } catch (e) { /* ignore */ }
    ...
    if (customId && customId.startsWith('music_admin_release_')) {
      try {
        ...
        // Log admin release
        // Лог админ-отключения тоже в ветку
        try { const threadCh = await client.channels.fetch('1446846080573771877').catch(()=>null); if (threadCh) await threadCh.send(`⛔️ Админ <@${user.id}> отключил плеер на сервере **${guild.name}** (владелец: <@${targetOwnerId}>)`); } catch(e){}
    ...
    if (customId === 'music_release') {
      try {
        ...
        // Лог остановки плеера — тоже в ветку
