...
// Simple in-memory player per guild
const players = new Map();
...
async function loadSavedQueues() {
  try {
    ...
    console.warn('loadSavedQueues failed', e && e.message);
  }
}

// initialize saved queues on module load
loadSavedQueues().catch(() => {});
...
async function streamFromUrl(url) {
  try {
    ...
    console.warn('streamFromUrl failed', e && e.message);
    return null;
  }
}


async function playNow(guild, voiceChannel, query, textChannel) {
  try {
    let url = await findYouTubeUrl(queryOrUrl);
    ...
    console.error('playNow error', e && e.message ? e.message : e);
    if (textChannel && textChannel.send) await textChannel.send('❌ Ошибка при воспроизведении.');
    return false;
  }
}
...
async function skip(guild, voiceChannel, textChannel) {
  const state = players.get(guild.id);
  if (!state) return false;
  try {
    state.player.stop();
...
async function changeVolume(guild, delta) {
  const state = players.get(guild.id);
  if (!state) return null;
  state.volume = Math.max(0.01, Math.min(5.0, (state.volume || 1.0) + delta));
  try {
    // update current resource volume if available
    // resource has inlineVolume
    return state.volume;
  } catch (e) {
    return state.volume;
  }
